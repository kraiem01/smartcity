...
def json_serializer(obj):
    if isinstance(obj,uuid.UUID):
        return str(obj)
    raise TypeError(f'object of type {obj.__class__.__main__} is not  JSON serializable')
...
def simulate_journey(producer,device_id):
    while True:
        ...
        if (vehicle_data['location'][0] >= BIRMINGHAM_COORDINATES['latitude'] and 
            ...
            print('vehicle has reached  Bringham Simulation ending...')
            break

        produce_data_to_kafka(producer,VEHICLE_TOPIC,vehicle_data)
        #produce_data_to_kafka(producer,GPS_TOPIC,gps_data)
        #produce_data_to_kafka(producer,TRAFIC_TOPIC,traffic_camera_data)
